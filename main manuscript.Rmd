...
```{r}
pdsi = raster('./data/pdsi.tif') %>% crop(ref)
pdsi = raster(vals=values(pdsi),
              ext=extent(ref), 
              nrows=dim(ref)[1],
              ncols=dim(ref)[2])
pdsi = setMinMax(pdsi)

#tm_shape(pdsi) + tm_raster(n = 10)
```
...
layers = stack(srtm, uspeed, vspeed, clearDays, days20, fapar, lightning, month100, pannual, pdriest, pdsi, pet, 
pseason, pwettest, tannual, tseason)
