...
eq.weibull = function(a = coef(res2)[1], b = coef(res2)[2], d = coef(res2)[3], x){
  return(d*((a/b) * ((x/b)^(a-1)) * exp(-(x/b)^a)))
}

estHeight = data.frame(srtm = seq(min(temp$srtm), max(temp$srtm), length.out = 100), altura_cop = eq.weibull(x = seq(min(temp$srtm), max(temp$srtm), length.out = 100)))
